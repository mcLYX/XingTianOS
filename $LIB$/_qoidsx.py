...
#解码配置设置
bytemin=const(8)#加载的最少字节数(8,不要改,除非QOI规范变了)
loadstep=const(96)#每次加载多少字节，更改可能影响速度
cachesize=const(64)#解码后的缓存大小，更改可能影响速度
cache565=b''#16位色缓存
dat=bytearray(2)
...
def ds(FileNAME):
  ...
  if f.read(4)!=b'qoif':
    ...
    scr.s()
    tft.setarea([0,0],[min(width-1,size[0]-1),min(size[1]-1,height-1)])
    channels=3#强制RGB
