import os
os.chdir('/$LIB$')
from _scr import Screen
scr=Screen(240,120,240*120*2)
f,s,t,r,c,_,i=scr.f,scr.s,scr.t,scr.r,scr.c,scr._,scr.i
...
def dino():
  ...
  while mp[0]>6 or h>mh[0]-5:#玩家没碰到障碍时
    ...
    pos+=bool(pos)*tuse
    f(0);_(0,116,240,4,1);r(0,108-int(h),8,8,1)#清屏绘制地面，玩家
    for j in range(len(mp)):
      mp[j]-=tuse*(2+pos//2000)
      _(int(mp[j]),116-mh[j],int(5+pos//1000),mh[j],1)#移动绘制障碍物
    if mp[0]<-3-pos/1000:
  ...
  t('Game Over',168,0);t('Tap Retry',168,9);t(str(hi),0,9);s();i(0);return hi
